...
void SetDesiredTqAndDegOfRtn() {
    ...
    //while (uiAttemptCntr < 10) {
        //uilclErrNo = fopen_s(&fpOut, "C:\\DND_Endurance\\TqDegCmd.txt", "wb");
        ...
        /*
        if (uilclErrNo == 0) {
            fseek(fpOut, 0, SEEK_SET);	//Go to beginning of file
            uiDesiredDegreeOfRtn = 0;
            StoreInteger(fpOut, uiDesiredDegreeOfRtn);
            fDesiredTq = 0;
            StoreFloat(fpOut, fDesiredTq);
            uiDesiredStatus = 0;
            uiModifyCmdStatusFlag = 1;
            StoreInteger(fpOut, uiDesiredStatus);
            uiDesiredDirectionOfRtn = 0;
            StoreInteger(fpOut, uiDesiredDirectionOfRtn);
            uiDesiredRPM = 0;
            StoreInteger(fpOut, uiDesiredRPM);
            fclose(fpOut);
            uiAttemptCntr = 30000;
            printf("\n Reset the file to avoid undesired motion!");
        }
        */
    //}
}

void GetDesiredTqAndDegOfRtn()	//VC++ Verification done
{

    //Fill up the buffered parameters
    uiDesiredDegreeOfRtn = 720;
    fDesiredTq = 5;
    uiDesiredStatus = CMD_STAT_LOADED;
    uiDesiredDirectionOfRtn = CW;
    uiDesiredRPM = 15;
    uiModifyCmdStatusFlag = 1;
    ...
    uint32 uiBuffDesiredDegreeOfRtn;
    float fBuffDesiredTq;
    uint32 uiBuffDesiredStatus;
    uint32 uiBuffDesiredDirectionOfRtn;
    uint32 uiBuffDesiredRPM;
    //printf("\nReading in");

    while (uiAttemptCntr < 10)
    {
        uilclErrNo = fopen_s(&fpIn, "C:\\DND_Endurance\\TqDegCmd.txt", "rb");
        if (uilclErrNo == 0)
        {

            uiBuffDesiredDegreeOfRtn = ReadInteger(fpIn, 0);
            //printf("\n R:%d",uiBuffDesiredDegreeOfRtn);
            fBuffDesiredTq = ReadFloat(fpIn, 4);
            //printf("\n T:%f",fBuffDesiredTq);
            uiBuffDesiredStatus = ReadInteger(fpIn, 8);
            //printf("\n sT:%d",uiBuffDesiredStatus);
            uiBuffDesiredDirectionOfRtn = ReadInteger(fpIn, 12);
            //printf("\n D:%d",uiBuffDesiredDirectionOfRtn);
            uiBuffDesiredRPM = ReadInteger(fpIn, 16);
            //printf("\n rpm:%d",uiBuffDesiredRPM);
            fclose(fpIn);
            uiAttemptCntr = 30000;
            //printf("\n Des Stat: %d", uiBuffDesiredStatus);
            if (uiBuffDesiredStatus == CMD_STAT_LOADED) {
                //printf("\n h2");
                if ((uiDesiredStatus == CMD_STAT_UNKNOWN) || (uiDesiredStatus == CMD_STAT_COMPLETED_POSN) || (uiDesiredStatus == CMD_STAT_REJECTED)) {
                    //printf("\n h3");
                    //Fill up the buffered parameters
                    uiDesiredDegreeOfRtn = uiBuffDesiredDegreeOfRtn;
                    fDesiredTq = fBuffDesiredTq;
                    uiDesiredStatus = uiBuffDesiredStatus;
                    uiDesiredDirectionOfRtn = uiBuffDesiredDirectionOfRtn;
                    uiDesiredRPM = uiBuffDesiredRPM;
                    uiModifyCmdStatusFlag = 1;
                    if (uiDesiredDirectionOfRtn == 1)
                        printf("\n Close: %f", fBuffDesiredTq);
                    else
                        printf("\n Open:%f", fBuffDesiredTq);
                    //!printf("\n\n Desired Status: %d", uiDesiredStatus);
                }
            }
        }
        else
        {
            uiAttemptCntr++;
            printf("\n GetDesiredTqAndDegOfRtn : File Open Error: %s", strerror(uilclErrNo));
        }
    }
    if (uiAttemptCntr == 10) {
        for (uiPrintCntr = 0; uiPrintCntr < 1000; uiPrintCntr++)
            printf("\n FAILED TO OPEN THE FILE: GetDesiredTqAndDegOfRtn");
    }
    */
}
...
void SetActualTqAndPosn()
{
    ...
    while (uiAttemptCntr < 10)
    {
        //uilclErrNo = fopen_s(&fpOut, "C:\\DND_Endurance\\servo.txt", "wb");
        if (uilclErrNo == 0)
        {
            //fseek(fpOut, 0, SEEK_SET);	//Go to beginning of file
            //StoreInteger(fpOut, uiActualPosn);
            //StoreInteger(fpOut, uiActualTq);
            //fclose(fpOut);
            uiAttemptCntr = 100;
        }
        else
        {
            uiAttemptCntr++;
            printf("\n SetActualTqAndPosn: File Open Error: %s", strerror(uilclErrNo));
        }
    }
    if (uiAttemptCntr == 10) {
        for (uiPrintCntr = 0; uiPrintCntr < 1000; uiPrintCntr++)
            printf("\n FAILED TO OPEN THE FILE: SetActualTqAndPosn");
    }
}
                ...
                for (i = 1; i <= 15000; i++)
                {
                    ...
                    if (wkc >= expectedWKC)
                    {
                         ...
                         */
                         ...
                         //printf(" PAV: %d, SFP: %d, Vel: %d, MaxTq: %dN-m", uiPosActualValue,uiSecondFeedbackPosn,uiVelocity,uiMaxTorque);
                        ...
                        if (chkCntr >= 20)
                        {
                            if (uiModifyCmdStatusFlag == 0) {
                                ...
                                chkCntr = 0;
                                printf("Checking Desired degree of Rotation: %d\n", uiDesiredStatus);
                            }
                        }
