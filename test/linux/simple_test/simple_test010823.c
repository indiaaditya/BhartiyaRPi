...
{//1
    ...
    printf("\nA");
    while (1) {
        //IOmap[0]++;       //Need to confirm that this needs to be removed.
      ...
      clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, &ts, &tleft);
      myTime = ts;
...
//    int16 i16val;
    ...
    {//2
        ...
        {//3
            ...
            if ((oloop == 0) && (ec_slave[0].Obits > 0)) oloop = 1; //If less than 8 bits assign 1 byte
            if (oloop > 8) oloop = 8;   //Not allowed more than 8 bytes
            
            iloop = ec_slave[0].Ibytes;
            if ((iloop == 0) && (ec_slave[0].Ibits > 0)) iloop = 1; //If less than 8 bits assign 1 byte
            if (iloop > 8) iloop = 8; //Not allowed more than 8 bytes
...
/*
            ...
            while(1){
                if(chk == 0)
                    ...
                    printf("Chk Val: %d", chk);
                    printf("\n State: %d", ec_slave[0].state);
                }
            }
*/
