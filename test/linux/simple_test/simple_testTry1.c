...
//    int16 i16val;
    ...
    if (ec_init(ifname))
    {
        ...
        if (ec_config_init(FALSE) > 0)
        {
                ...
                for (slc = 1; slc <= ec_slavecount; slc++)
                {
                    ...
                    if ((ec_slave[slc].eep_man == MANUFACTURER_ID) && (ec_slave[slc].eep_id == PRODUCT_ID))
                    {
                        printf("Slave:%d Name:%s Output size:%3dbits Input size:%3dbits State:%2d delay:%d.%d\n",
                               ...
                               ec_slave[slc].state, (int)ec_slave[slc].pdelay, ec_slave[slc].hasdc);
                        printf("         Out:%8.8x,%4d In:%8.8x,%4d\n",
                               (int)ec_slave[slc].outputs, ec_slave[slc].Obytes, (int)ec_slave[slc].inputs, ec_slave[slc].Ibytes);

                        printf("It gives us great pleasure to inform you that Panasonic Ethercat Servo has been found!!!\n");
                        printf("Found %s at position %d\n", ec_slave[slc].name, slc);
                        // link slave specific setup to preop->safeop hook
                        //Step 3: configure distributed clock    
            ...
            printf("\n Sent process data once successfully");
