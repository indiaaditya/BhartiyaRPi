...
#define MAX_MOTOR_TORQUE  (GEAR_RATIO * MOTOR_TORQUE_RATING) 
...
int convertTorqueToPercentage(float rTorque);
            ...
            else
            {
                ...
                while (1)
                {
                    ...
                    if (MotorOpnStatus == 1)
                    {
                        ...
                        if(SerialCommand.direction == ACW){
                            motionParams.iTgtPosn = iPosActualValue.hl + (ENCODER_CNT_PER_DEGREE_POST_GEARBOX * (SerialCommand.degreeOfRtn + 20));
                        }
                        else{
                            motionParams.iTgtPosn = iPosActualValue.hl - (ENCODER_CNT_PER_DEGREE_POST_GEARBOX * SerialCommand.degreeOfRtn);
                        }    
                        motionParams.iMaxTq = convertTorqueToPercentage(SerialCommand.torque);
                        /*
                        ...
                        */
                        ...
                        fillMotionParams(SerialCommand.rpm, motionParams.iMaxTq, motionParams.iTgtPosn);
...
void serialTransact(){
   // char lclCmdClose[] = "CCC";
    //char lclCmdOpen[] = "OOO";
    int lclRetVar = 0;
    //printf("\rStatus:%d",serial.transactStatus);
    ...
    switch(serial.transactStatus){
        ...
        case CHECK_DATA_RECEIVED:
            ...
            //printf("\nrxCntr:%d",serial.rxByteCntr);
            if(serial.rxByteCntr == 18){
                lclRetVar = extractSerialCmdData(serial.rxBuffer);
                if(lclRetVar == 1){
                    MotorOpnStatus = 1;
                    serial.transactStatus = TRANSMIT_TQ_AND_POSN_BEGIN;
                }
                else{
                    printf("Rejection Reason: %d",lclRetVar);
                }
...
int convertTorqueToPercentage(float rTorque){
    int retPercentage;
    retPercentage = (int)((rTorque *  1000)/MAX_MOTOR_TORQUE);   
